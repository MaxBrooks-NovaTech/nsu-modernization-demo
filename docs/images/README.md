...
| `dbt-docs-fact-enrollment-columns.png` | `fact_enrollment`'s real column list, types, descriptions, and test badges, as dbt actually reports them | Same dbt docs site, `fact_enrollment` model page, "Columns" tab |
...
| `postgres-tables.png` | The actual `raw` and `analytics` schema tables in the running PostgreSQL container, `fact_enrollment`'s real column/type list, and 8 real sample rows | A local HTML rendering of real `psql` output (`docker compose exec postgres psql ...`) — see note below |
| `dbt-build-log.png` | A real `dbt build` terminal run: model creation, `fact_enrollment`'s grain/quality tests, and the final `PASS=62 WARN=0 ERROR=0` summary | Captured `dbt build` output on 2026-08-16, rendered as a terminal-style block — see note below |
...
| `data-contract.png` | The full `fact_enrollment` data contract: version, status, owner, steward, grain, freshness target, required fields, required tests, breaking-change rules, and consumer | A local HTML rendering of `contracts/fact_enrollment.yml`'s real content — see note below |
...
| `Executive Enrollment & Admissions.png` | The live Power BI report's first page: term/program/school slicers, 5 KPI cards (Applications 272, Admits 201, Deposits 109, Enrolled 1,897, Yield 54.2%), an Enrolled-by-School column chart, and a 3-stage recruitment funnel — a native Power BI Service screenshot, not a rendering | Power BI Service, report connected live to the `nsu_demo` Fabric Lakehouse SQL endpoint |
| `Institutional Data Trust.png` | The live report's second page: product/status slicers, Certification Status and Release Gate cards, the Certified Products table (all 3 marts, Certified), and real `dbt build` test results per column | Power BI Service, same live connection |
| `Data Lineage & Certification.png` | The live report's third page: product/consumer/status slicers, the Certified Product Lineage table (source entities → model → semantic definitions → consumers), and the Certification Release Details table | Power BI Service, same live connection |
| `Metric Definitions and Measure Code.png` | The `measure_table`'s real data: all 9 governed DAX measures with their exact formula text, rendered as an actual Power BI table (not copied from the TMDL file) | Power BI Service, same live connection — confirms the `DATATABLE`-sourced `measure_table` in `NSU BI Modernization Demo.SemanticModel` renders correctly |

## Why some of these are "rendered," not native screenshots

dbt Core's docs site only renders resources declared in its own schema (models, sources, tests, macros). `semantic/metric_definitions.yml` and `contracts/fact_enrollment.yml` are custom governance artifacts this project maintains outside dbt's schema (see `docs/architecture.md`), so neither has a native dbt docs page. Likewise, there is no GUI database client installed in this environment to screenshot directly. For all of these, the fix was **not** to skip them or fake a UI that doesn't exist — it was to render the *real* file/query/log/command-output content as a simple local HTML page and screenshot that, clearly labeled as such directly on each image. Every number and message in `semantic-metric-definitions.png`, `postgres-tables.png`, `dbt-build-log.png`, `dashboard-quality-metrics-summary.png`, `data-contract.png`, and `change-management-detection.png` is copied verbatim from a real file, a real `psql` session, a real `dbt build` run, or a real invocation of `scripts/check_contract_changes.py`; nothing is illustrative or approximated. `dashboard-quality-metrics-summary.png` in particular is explicitly labeled on the image itself as "not a native Power BI screenshot" so it's never mistaken for one.

`Executive Enrollment & Admissions.png`, `Institutional Data Trust.png`, `Data Lineage & Certification.png`, and `Metric Definitions and Measure Code.png` are different: they are native Power BI Service screenshots of the actual finished report, connected live to the `nsu_demo` Fabric Lakehouse SQL endpoint — not rendered stand-ins. `dashboard-quality-metrics-summary.png` predates these and is kept as the earlier rendered mockup, made when Power BI Desktop wasn't available in the build environment; it's now superseded by the real `Institutional Data Trust.png` screenshot above.
